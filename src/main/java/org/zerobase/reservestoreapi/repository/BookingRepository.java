...
public interface BookingRepository extends
    ...
    default void customize(QuerydslBindings bindings, QBooking root) {
        ...
        bindings.bind(root.approve).as("approve").first(((path, value) -> path.isNull().or(path.eq(value))));
        bindings.bind(root.createdAt).as("date").first(((path, value) -> path.between(value, value.plusDays(1).minusSeconds(1))));
    }
