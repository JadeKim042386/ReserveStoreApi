...
public class BookingApi {
    ...
    /**
     * Look up booking info for a specific date for a specific store.
     ...
     * 1. if requested user wasn't sign up
     * 2. if already exists booking at request booking time
     * 3. if already exists booking by request user
     */
    @PostMapping("/{storeId}")
    public ResponseEntity<?> requestBooking(
            @PathVariable Long storeId,
            @RequestParam("time") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime requestBookingTime,
    ...
    /**
     * store can deny or approve the booking.
     * if the requested store and the booking store do not match, throw exception.
     */
    ...
    /**
     * when a user visits the store, can request a visit check.
     * if the user wasn't arrive 10 minutes earlier than the booking time, throw exception.
     */
